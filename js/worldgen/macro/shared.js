(() => {
    ...
    const moduleRegistry = macro._moduleRegistry instanceof Map
        ? macro._moduleRegistry
        : new Map();
    const pipelineRegistry = macro._pipelineRegistry instanceof Map
        ? macro._pipelineRegistry
        : new Map();
    const PHASE_ID = 'phase1';
    const PHASE_VERSION = 'phase1-v1';
    ...
    function createTodoContractedError(entryId) {
        const error = new Error(`[worldgen/macro] ${entryId} is a TODO CONTRACTED stub.`);
    ...
    function registerModule(moduleId, descriptor = {}) {
        if (typeof moduleId !== 'string' || !moduleId.trim()) {
            throw new Error('[worldgen/macro] moduleId is required.');
        }
    ...
    function registerPipelineStep(stepId, descriptor = {}) {
        if (typeof stepId !== 'string' || !stepId.trim()) {
            throw new Error('[worldgen/macro] stepId is required.');
        }
